...
def main():
    ...
    jx_list = load_jx_list("glove/vocab.txt")

    train_size = int(0.8 * len(X))
    X_train, X_val = X[:train_size], X[train_size:]
    Y_train, Y_val = Y[:train_size], Y[train_size:]

    embedding = torch.nn.Embedding.from_pretrained(get_glove_embedding(16, "vectors.txt"), freeze=False)
    ...
    # Save best models
    torch.save(encoder, "checkpoints/beat_encoder.pth")
    torch.save(decoder, "checkpoints/beat_decoder.pth")
